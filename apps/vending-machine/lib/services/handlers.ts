...
export const weatherHandler: VendingHandler = async (_req, query) => {
  ...
  const geoJson = (await geo.json()) as { results?: { latitude: number; longitude: number; name: string }[] };
  const hit = geoJson.results?.[0];
  if (!hit) return { city, error: "city_not_found" };
...
export const cryptoPricesHandler: VendingHandler = async (_req, query) => {
  ...
  return { ids: ids.split(","), prices: await res.json() };
};
...
export const qrGeneratorHandler: VendingHandler = async (_req, query) => {
  ...
  const size = Math.min(512, Math.max(128, Number(query.size ?? 256)));
